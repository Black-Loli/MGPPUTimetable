...
<head>
    <meta charset="UTF-8">
    <title> Месяц | Группы в корпусе </title>
...
<script>
    ...
    function generateLessons(date, lessons) {
        ...
        _(lessons.sort(function (lesson1, lesson2) {
            ...
            const groups = _(lessonRangeForLessonNumber).map('GroupCode').uniq().value().map(function(group){
                return `<h2 class="groups_name group_name__month">${group}</h2>`;
            })
